...
/********************* setting timeout for receiving file to avoid waiting in infinite time ***********/
	...
	printf("Set sock opt: %d",setsockopt(socket_cli, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,sizeof(struct timeval)));
...
/******************************************************************************************************/
	...
	//n = recvfrom(socket_cli,mesg,100,0,(struct sockaddr *)&addr_cli,&clilen);
		...
		if(n<0){
			...
			printf("Odebrany rozmiar pliku %d, aaa= %d, bbb=%d\n", rsize,aaa,bbb);
...
start:	
	...
	//if(PID != 0) return 0;
	tv.tv_sec = 0;
	setsockopt(socket_cli, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,sizeof(struct timeval));
